...
SGameMain::SGameMain()
{
	...
	//通关触发器==============================================================================
	m_pLevelGate		=	new CSprite("levelgate");
	//========================================================================================
	...
	//射手精灵================================================================================
	m_pShooter			=	new CSprite("shooter1");
	//预设传送门精灵==========================================================================
	...
	m_pTest3			=	new CPlayerSpriteA("testC3");
	//========================================================================================
...
void SGameMain::GameMainLoop( float	fDeltaTime )
{
	switch( GetGameState() )
	{
		// 初始化游戏，清空上一局相关数据
...
void SGameMain::GameInit()
{
	...
	m_pTest3->PlayerSpriteInit(1);
	//========================================================================================
	m_pMap.SetMapInfo(14, 14);
	//========================================================================================
	m_iPlayerAState = 0;
	m_iPlayerBState = 0;
	//========================================================================================
	...
	m_pPrePortalA1->SetDir(3);
	m_pPrePortalB1->SetDir(0);
	m_pPrePortalA2->SetDir(0);
	m_pPrePortalB2->SetDir(3);
	//========================================================================================
...
//
// 每局游戏进行中
void SGameMain::GameRun( float fDeltaTime )
{
	//开场渐变效果
	...
	if(m_pTrigger1->GetTriggerResult() == 1){
		m_pPrePortalB1->SetSpritePosition(-270, 330);
	}
	if(m_pTrigger2->GetTriggerResult() == 1){
		m_pDoor2->SpriteMoveTo(fDoor2PosX, fDoor2PosY - 140, 40, 1);
	}
	if(m_pTrigger3->GetTriggerResult() == 1){
		m_pDoor3->SpriteMoveTo(fDoor3PosX, fDoor3PosY - 140, 40, 1);
	}
	//玩家触发机关============================================================================
	if(m_pTrigger4->GetTriggerResult() == 1){
		m_pDoor1->SpriteMoveTo(fDoor1PosX, fDoor1PosY + 160, 40, 1);
	}
	//玩家死亡============================================================================
	if(m_iIfAlive == 0){
...
void SGameMain::OnMouseMove( const float fMouseX, const float fMouseY )
{
...
// 参数 fMouseX, fMouseY：为鼠标当前坐标
void SGameMain::OnMouseClick( const int iMouseType, const float fMouseX, const float fMouseY )
{
	if(m_iGameState == 2){
		if(iMouseType == 0){
			AddBulletA();
...
// 参数 iMouseType：鼠标按键值，见 enum MouseTypes 定义
// 参数 fMouseX, fMouseY：为鼠标当前坐标
void SGameMain::OnMouseUp( const int iMouseType, const float fMouseX, const float fMouseY )
{
...
void SGameMain::OnKeyDown( const int iKey, const bool bAltPress, const bool bShiftPress, const bool bCtrlPress )
{	
	//==============================================================
	if(GetGameState() == 0){
		if(iKey == KEY_SPACE){
...
void SGameMain::OnKeyUp( const int iKey )
{
	if(GetGameState() == 2){
...
void SGameMain::OnSpriteColSprite( const char *szSrcName, const char *szTarName )
{
	//========================================================================================
...
//CPlayer类型
	...
	if(strstr(szSrcName, "testC1") != NULL && strstr(szTarName, "trigger") != NULL){
	...
	//土豆3部分===============================================================================
	if(strstr(szSrcName, "testC3") != NULL && strstr(szTarName, "Block_L") != NULL){
		m_pTest3->SetSpriteLinearVelocityY(0);
	}									//土豆触碰地图
	if(strstr(szSrcName, "testC3") != NULL && strstr(szTarName, "portal") != NULL){
		if(strcmp(szTarName, "portalA") == 0){
			Transmission(m_pPortalB, m_pTest3);
		}
		if(strcmp(szTarName, "portalB") == 0){
			Transmission(m_pPortalA, m_pTest3);
		}
	}									//土豆被门传送
	if(strstr(szSrcName, "testC3") != NULL && strstr(szTarName, "trigger") != NULL){
...
//门会组织角色闯过
//可由机关出发打开
	if(strstr(szSrcName, "test") != NULL && strstr(szTarName, "door") != NULL){
		m_pPlayerSpriteA->SetSpriteLinearVelocityX(0);
		m_pPlayerSpriteB->SetSpriteLinearVelocityX(0);
	}
//玩家与关卡出口碰撞==========================================================================
...
void SGameMain::OnSpriteColWorldLimit( const char *szName, const int iColSide )
{

}
//============================================================================================
void SGameMain::AddBulletA()
{	
...
{	
	...
	tmpBullet->OnMove(m_pShooter->GetSpritePositionX(), m_pShooter->GetSpritePositionY(), m_pShooter->GetSpritePositionX() + 1, m_pShooter->GetSpritePositionY());
																		//调用OnMove() 设置子弹的初始位置 子弹的方向 子弹的速度
	m_iBulletNum_3++;													//子弹个数加1
}
//============================================================================================
CSprite* SGameMain::FindBulletA(const char* szName)
{
...
CSprite* SGameMain::FindBulletB(const char* szName)
{
...
CSprite* SGameMain::FindBulletC(const char* szName)
{
...
//============================================================================================
void SGameMain::OnBulletHitA(const char* szSrcName, const char*szTarName)
{
	m_SoundPortal->PlaySoundA();
...
void SGameMain::OnBulletHitB(const char* szSrcName, const char*szTarName)
{
	m_SoundPortal->PlaySoundA();
	float fBulletPosX, fBulletPosY, fBlockPosX, fBlockPosY;
...
void SGameMain::OnBulletHitC(const char* szSrcName, const char*szTarName)
{
	FindBulletC(szSrcName)->DeleteSprite();
}
//============================================================================================
void SGameMain::Transmission(CPortal* pPortal, CPlayer* pPlayerSprite){
...
// 参数 Accuracy：变化精度
int SGameMain::ColorAlphaMaking(const double AlphaStart,const double AlphaEnd,const double Accuracy)
{
